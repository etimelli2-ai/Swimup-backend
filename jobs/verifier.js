...
async function attendreResultat(requestId, apiKey, maxTentatives = 15) {
  for (let i = 0; i < maxTentatives; i++) {
    ...
    try {
      ...
      if (data?.status === 'Error') return { success: false, error: true, raison: 'Erreur Outscraper' };
    } catch (e) {
...
async function verifierViaOutscraper(lienMaps, texteAttendu, nbEtoilesAttendu) {
  try {
    ...
    const query = lienMaps;
    console.log(`📍 Query Outscraper: ${query}`);

    const response = await axios.get('https://api.app.outscraper.com/maps/reviews-v3', {
      params: { query, reviewsLimit: 50, language: 'fr', sort: 'newest', async: true },
      headers: { 'X-API-KEY': apiKey },
      timeout: 30000,
    });
    ...
    if (data?.status === 'Success') {
      ...
      if (result.error) return { trouve: false, erreur: true, raison: result.raison };
...
async function jobVerificationQuotidienne() {
  ...
  try {
    const verifRes = await db.execute({
      sql: `SELECT v.*, a.lien_maps, a.texte, a.nb_etoiles, a.prix, a.client_id, a.valide_at, a.delai_paiement
    ...
    for (const verif of verifRes.rows) {
      try {
        ...
        const result = await verifierViaOutscraper(verif.lien_maps, verif.texte, verif.nb_etoiles);
        ...
        if (!result.trouve) {
          ...
          await db.execute({ sql: "UPDATE avis SET statut = 'refuse' WHERE id = ?", args: [verif.avis_id] });
          await db.execute({ sql: "UPDATE verifications SET statut = 'inactif' WHERE id = ?", args: [verif.id] });
          ...
          if (transRes.rows.length) {
            const montant = transRes.rows[0].montant;
            await db.execute({ sql: 'UPDATE users SET solde = MAX(0, solde - ?) WHERE id = ?', args: [montant, verif.user_id] });
            await db.execute({
          ...
          await db.execute({
            ...
            args: [verif.user_id, 'Avis supprimé', 'Ton avis n'a pas été trouvé sur Google Maps.'],
          });
          continue;
        }
        ...
        if (joursEcoules >= verif.delai_paiement) {
          ...
          if (!dejaCredite.rows.length) {
            await db.execute({ sql: 'UPDATE users SET solde = solde + ? WHERE id = ?', args: [verif.prix, verif.user_id] });
            await db.execute({
            ...
            await db.execute({ sql: "UPDATE avis SET statut = 'paye', paye_at = CURRENT_TIMESTAMP WHERE id = ?", args: [verif.avis_id] });
            await db.execute({ sql: "UPDATE verifications SET statut = 'paye' WHERE id = ?", args: [verif.id] });
            await db.execute({
              sql: 'INSERT INTO notifications (user_id, titre, message) VALUES (?,?,?)',
